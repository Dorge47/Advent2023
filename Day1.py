    ...
    for character in line:
        if character.isnumeric():
            if not firstNumFound:
                ...
                firstNumFound = True
            lastNumFound = character
    calibrationValue += lastNumFound
    totalSum += int(calibrationValue)
print(totalSum)
def get_calibration_value(inputLine):
    minIndexList = [[]] * 20
    maxIndexList = [[]] * 20
    tempMin = []
    tempMax = []
    for i in range(0,10):
        minIndexList[i] = [inputLine.find(str(i)),i]
        maxIndexList[i] = [inputLine.rfind(str(i)),i]
    minIndexList[10] = [inputLine.find("zero"),0]
    minIndexList[11] = [inputLine.find("one"),1]
    minIndexList[12] = [inputLine.find("two"),2]
    minIndexList[13] = [inputLine.find("three"),3]
    minIndexList[14] = [inputLine.find("four"),4]
    minIndexList[15] = [inputLine.find("five"),5]
    minIndexList[16] = [inputLine.find("six"),6]
    minIndexList[17] = [inputLine.find("seven"),7]
    minIndexList[18] = [inputLine.find("eight"),8]
    minIndexList[19] = [inputLine.find("nine"),9]
    maxIndexList[10] = [inputLine.rfind("zero"),0]
    maxIndexList[11] = [inputLine.rfind("one"),1]
    maxIndexList[12] = [inputLine.rfind("two"),2]
    maxIndexList[13] = [inputLine.rfind("three"),3]
    maxIndexList[14] = [inputLine.rfind("four"),4]
    maxIndexList[15] = [inputLine.rfind("five"),5]
    maxIndexList[16] = [inputLine.rfind("six"),6]
    maxIndexList[17] = [inputLine.rfind("seven"),7]
    maxIndexList[18] = [inputLine.rfind("eight"),8]
    maxIndexList[19] = [inputLine.rfind("nine"),9]
    for i in range(0,20):
        if minIndexList[i][0] != -1:# Ignore values not found
            if tempMin == []:# Define reference point if not already defined
                tempMin = minIndexList[i]
            elif minIndexList[i][0] < tempMin[0]:
                tempMin = minIndexList[i]
        if maxIndexList[i][0] != -1:
            if tempMax == []:
                tempMax = maxIndexList[i]
            elif maxIndexList[i][0] > tempMax[0]:
                tempMax = maxIndexList[i]
    tempOutput = str(tempMin[1]) + str(tempMax[1])# Concatenate
    return int(tempOutput)
totalSum = 0
for line in puzzleInput:
    totalSum += get_calibration_value(line)
print(totalSum)